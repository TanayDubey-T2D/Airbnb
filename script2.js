...
const _listingTemplate = `<div class="card listing">
...
class FunctionAnalysis {
  constructor(functionString) {
    try {
      ...
      this.rootNode = esprima.parseScript(functionString, {
        range: true,
        tolerant: true,
      });
      ...
      if (
        this.rootNode.body[0].type != "FunctionDeclaration" ||
  ...
  findConditionals(node) {
    ...
    let matches = body.filter((s) => s.type == "IfStatement");
    return matches;
  }
  ...
  getText(node) {
    ...
    return this.functionString.slice(node.range[0], node.range[1]);
  }
}
...
function cityAnalysis() {
  ...
  if (
    arrayDeclarationStatements.length === 0 ||
    ...
    console.log(
      ...
      constructHintMessage(
        1,
        "We are detecting that you may not have declared an empty array variable. Make sure you initialized an empty array and assigned it to a variable. Later we will use it to store our matching indices.",
        RESOURCES.arrays
      )
    );
    return false;
  ...
  if (returnDeclarationStatements.length === 0) {
    console.log(
      ...
      constructHintMessage(
        5,
        `We are detecting that you may not have made a return statment. After your for loop you need to ${COLOR_CODE.bright}return${COLOR_CODE.reset}${COLOR_CODE.cyan} the variable that represents your array of matches.`,
...
function filterByCityTest_singleMatch() {
  ...
  if (exception != undefined) {
    cityErrorMessage = constructErrorMessage(
      TEST_NAME,
      "?",
    ...
    cityErrorMessage = constructErrorMessage(
      TEST_NAME,
      "?",
      `This  function should return an array but got type '${typeof result}'.`
    );
    return false;
...
function filterByCityTest() {
  ...
  if (passedSingleMatch && passedNoMatch && passedDoubleMatch) {
    console.log(
      `${COLOR_CODE.bright}filterByCityTest passed! ${COLOR_CODE.underscore}${COLOR_CODE.green}Great job!${COLOR_CODE.reset}`
    );
    return true;
  } else {
    cityAnalysis();
...
function typeAnalysis() {
  ...
  if (conditionalStatements.length === 0) {
    console.log(
      ...
      constructHintMessage(
        3,
        `We are detecting that you may not have a conditional to check for matches. Create an ${COLOR_CODE.bright}if Statement${COLOR_CODE.reset}${COLOR_CODE.cyan} inside of your ${COLOR_CODE.bright}for loops${COLOR_CODE.reset}${COLOR_CODE.cyan} that will compare every index of targetTypes to each index of listingTypesArray, and run your code block if that comparson equates to true.`,
        RESOURCES.conditionals
      )
    );
    return false;
...
function filterByTypesTest_singleMatch() {
  ...
  if (exception != undefined) {
    typeErrorMessage = constructErrorMessage(
      TEST_NAME,
      "?",
      `We encountered an error running your function.\n\n${exception}`
    );
...
function filterByTypesTest_noMatch() {
  ...
  if (exception != undefined) {
    ...
    typeErrorMessage = constructErrorMessage(
      TEST_NAME,
      "?",
      `This  function should return an but got type '${typeof result}'.`
    );
    return false;
  } else if (result.length != 0) {
    typeErrorMessage = constructErrorMessage(
...
function filterByTypesTest() {
  ...
  if (passedSingleMatch && passedNoMatch && passedDoubleMatch) {
    console.log(
      `${COLOR_CODE.bright}filterByTypesTest passed! ${COLOR_CODE.underscore}${COLOR_CODE.green}Great job!${COLOR_CODE.reset}`
    );
    return true;
  } else {
    typeAnalysis();
...
function _logfilterByPriceTestParameters(
  minPrice,
  maxPrice,
  listingPriceArray
) {
...
function filterByPriceTest_singleMatch() {
  ...
  if (exception != undefined) {
    priceErrorMessage = constructErrorMessage(
      TEST_NAME,
      "?",
      `We encountered an error running your function.\n\n${exception}`
    );
...
function filterByPriceTest() {
  ...
  if (passedSingleMatch && passedNoMatch && passedDoubleMatch) {
    console.log(
      `${COLOR_CODE.bright}filterByPriceTest passed! ${COLOR_CODE.underscore}${COLOR_CODE.green}Great job!${COLOR_CODE.reset}`
    );
    return true;
  } else {
    priceAnalysis();
...
function runAllTests() {
  let tests = [filterByCityTest, filterByTypesTest, filterByPriceTest];
  let passed = true;
